...
{
    public partial class OvertimeDetail : ComponentBase
    {
        ...
        [Parameter] public EventCallback<Domain.Overtime> OpenApprovedModal { get; set; }
        [Parameter] public EventCallback<Domain.Overtime> OpenDeclinedModal { get; set; }
        [Parameter] public EventCallback<Domain.Overtime> OpenCancelApproveModal { get; set; }
        List<Users> StaffDatas = new List<Users>();
        List<Users> StaffSearchDatas = new List<Users>();
        OvertimeEditModel editModel = new OvertimeEditModel();
        InputWatcher inputWatcher;
        Domain.Overtime selectModel = new Domain.Overtime();
        bool addNew = false;
        ...
        public void LoadEditModel(Domain.Overtime value, bool readOnly = false)
        {
            try
            {
                ...
                editModel = Mapper.Map<OvertimeEditModel>(value);
        ...
        async Task AddAsync()
        {
            try
            {
                editModel = new OvertimeEditModel();
        ...
        async Task SaveAsync()
        {
            try
            {
                ...
                var vacation = Mapper.Map<Domain.Overtime>(editModel);
                if (vacation.Id.IsNotNullOrEmpty())
                {
                    result = await OvertimeService.UpdateAsync(vacation);
                }
                else
                {
                    vacation.Id = ObjectExtentions.GenerateGuid();
                    vacation.CreateDate = DateTime.Now;
                    result = await OvertimeService.AddAsync(vacation);
                }
                if (result)
                {
